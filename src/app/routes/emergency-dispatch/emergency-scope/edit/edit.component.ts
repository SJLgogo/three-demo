  ...
  selectUser(which:any) {
    ...
    if (this.i.id != '') {
      ...
      if (which == 1 && this.relevanceScope != null) {
        ...
        this.checkScope.map((item:any) => {
          ...
          obj['corpId'] = item['corpId'];
          obj['projectName'] = item['name'].split('_')[0] + '_';
          obj['name'] = item['name'].split('_')[1];
          selectedItems.push(obj);
        });
      }
    ...
    this.modal
      ...
      .subscribe(res => {
        ...
        console.log('res：', res);
        selectedItems.forEach(function (value:any, index:any, array:any) {
          if (value.category == 'employee') {
            Employees.push({
              name: value.projectName + "_" + value.name,
              corpId: value.corpId,
              employeeId: value.id,
              thirdId: value.id,
              type: 1,
            ...
            Employees.push({ name: value.name, corpId: value.corpId, thirdId: value.id, type: 0 });
          }
        })
